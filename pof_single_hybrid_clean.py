...
def temporal_train_test_split(
    ...
    """Split based on installation date - prevents future leakage"""
    ...
    df_sorted = df_sorted.sort_values("_install_clean").reset_index(drop=True)
    
    cutoff_date = df_sorted["_install_clean"].quantile(1 - test_size)
    train_mask = df_sorted["_install_clean"] <= cutoff_date
    
    train_idx = df_sorted[train_mask].index.values
    test_idx = df_sorted[~train_mask].index.values
    
    if logger:
        logger.info(f"[TEMPORAL] Cutoff: {cutoff_date.date()} | Train: {len(train_idx)} | Test: {len(test_idx)}")
        if "event" in df.columns:
            train_rate = df.loc[train_idx, "event"].mean()
            test_rate = df.loc[test_idx, "event"].mean()
            logger.info(f"[TEMPORAL] Train events: {train_rate:.1%} | Test events: {test_rate:.1%}")
...
def build_survival_base(
    ...
    """Create survival dataset with event/duration"""
    
    # First failure per equipment
    first_fail = df_fault.groupby("cbs_id")["started at"].min().rename("Ilk_Ariza_Tarihi")
    
    # Keep essential columns for downstream processing
    ...
    df = df.merge(first_fail, on="cbs_id", how="left")
    
    # Event = 1 if equipment failed
    df["event"] = df["Ilk_Ariza_Tarihi"].notna().astype(int)
    
    # Duration = time to first failure (or censoring at data_end_date)
    df["duration_days"] = np.where(
        df["event"] == 1,
        (df["Ilk_Ariza_Tarihi"] - df["Kurulum_Tarihi"]).dt.days,
        (data_end_date - df["Kurulum_Tarihi"]).dt.days
    )
    
    df = df[df["duration_days"] > 0].copy()
    df["duration_days"] = df["duration_days"].clip(upper=60*365)  # Cap at 60 years
    
    logger.info(f"[SURVIVAL] Base dataset: {len(df)} | Events: {df['event'].sum()} ({100*df['event'].mean():.1f}%)")
    return df
...
def build_structural_features(
    ...
    """Leakage-safe structural features (time-invariant)"""

    # Note: These columns are already in survival_base, so exclude them here to avoid duplicates
    survival_base_cols = ["Kurulum_Tarihi", "Ekipman_Tipi", "Gerilim_Sinifi", "Marka", "Fault_Count"]
    cols = [c for c in FEATURE_REGISTRY["structural_features"]
            if c in equipment_master.columns and c not in survival_base_cols]

    # Must include cbs_id for merging
    if "cbs_id" not in cols:
        cols = ["cbs_id"] + cols

    df = equipment_master[cols].drop_duplicates("cbs_id").copy()

    logger.info(f"[FEATURES] Structural: {len(df.columns)-1} columns, {len(df)} assets")  # -1 for cbs_id
    return df
...
def build_temporal_features(
    ...
    """Time-dependent features (snapshot at t_ref)"""
    
    # Note: Kurulum_Tarihi is NOT included - it's already in survival_base
    # We only calculate age-derived features here
    df = equipment_master[["cbs_id", "Kurulum_Tarihi"]].copy()
    df["Tref_Yas_Gun"] = (t_ref - df["Kurulum_Tarihi"]).dt.days.clip(lower=0)
    df["Tref_Ay"] = t_ref.month
    
    # Drop Kurulum_Tarihi - we only needed it for age calculation
    df = df.drop(columns=["Kurulum_Tarihi"])
    
    # Merge chronic
    if chronic_df is not None:
        df = df.merge(chronic_df, on="cbs_id", how="left")
    
    logger.info(f"[FEATURES] Temporal: {len(df.columns)} columns")
    return df
...
def select_cox_safe_features(df: pd.DataFrame, structural_cols: list, logger: logging.Logger) -> pd.DataFrame:
    """Prepare features for Cox model (with Kurulum_Tarihi for temporal split)"""
    base_cols = select_survival_safe_features(df, structural_cols, logger)
    
    # Add Kurulum_Tarihi for temporal split
    ...
    # Preserve Kurulum_Tarihi before encoding
    kurulum_col = None
    if "Kurulum_Tarihi" in X.columns:
        kurulum_col = X["Kurulum_Tarihi"].copy()
        X = X.drop(columns=["Kurulum_Tarihi"])
    
    # One-hot encode categoricals (drop high-cardinality)
    cat_cols = X.select_dtypes(include="object").columns.tolist()
    for col in cat_cols[:]:
        if X[col].nunique() > 20:
            logger.warning(f"[COX] Dropping {col}: high cardinality")
...
def train_cox_weibull(
    ...
    """Train Cox + Weibull survival models with temporal validation"""
    if not LIFELINES_OK:
        return None, None
    ...
    work = work.select_dtypes(include=[np.number]).apply(pd.to_numeric, errors="coerce").fillna(0)
    ...
    try:
        ...
        logger.warning(f"[COX] Temporal split failed ({e}), using random")
        train_idx, test_idx = train_test_split(
            np.arange(len(work)), test_size=0.25, random_state=42, stratify=event.values
        )
    
    # Drop Kurulum_Tarihi before training
    work = work.drop(columns=["Kurulum_Tarihi"], errors="ignore")
    train_data = work.iloc[train_idx]
    test_data = work.iloc[test_idx]
...
def train_rsf_survival(
    ...
    """Train Random Survival Forest with temporal validation"""
    if not SKSURV_OK:
        return None
    ...
    # Temporal split
    try:
        train_idx, test_idx = temporal_train_test_split(df, test_size=0.25, logger=logger)
    except Exception as e:
        logger.warning(f"[RSF] Temporal split failed ({e}), using random")
        train_idx, test_idx = train_test_split(
            np.arange(len(X)), test_size=0.25, random_state=42, stratify=df["event"].values
        )
...
class TemporalBacktester:
    ...
    def _generate_snapshot(self, cutoff_date: pd.Timestamp):
        """Create training dataset as if standing at cutoff_date"""
        ...
        survival_base = build_survival_base(equipment_master, faults_past, self.logger, cutoff_date)
        
        feat_structural = build_structural_features(equipment_master, self.logger)
        chronic_df = compute_chronic_features(faults_past, cutoff_date, self.logger)
        feat_temporal = build_temporal_features(equipment_master, chronic_df, cutoff_date, self.logger)
        
        features = feat_structural.merge(feat_temporal, on="cbs_id", how="left")
        df_snapshot = survival_base.merge(features, on="cbs_id", how="left")
...
def main():
    ...
    # =============================================================================
    # PRODUCTION PIPELINE (Full history)
    # =============================================================================
    logger.info("="*60)
    ...
    logger.info("[STEP 2] Building equipment master...")
    equipment_master = build_equipment_master(df_fault, df_healthy, logger, data_end_date)
    survival_base = build_survival_base(equipment_master, df_fault, logger, data_end_date)
    
    # Feature engineering
    logger.info("[STEP 3] Engineering features...")
    feat_structural = build_structural_features(equipment_master, logger)
    chronic_df = compute_chronic_features(df_fault, data_end_date, logger)
    feat_temporal = build_temporal_features(equipment_master, chronic_df, data_end_date, logger)
    
    features_all = feat_structural.merge(feat_temporal, on="cbs_id", how="left")
    
    # Merge survival + features
    # No duplicates expected since Kurulum_Tarihi and Ekipman_Tipi only in survival_base
    df_all = survival_base.merge(features_all, on="cbs_id", how="left")

    # structural_cols should include all structural features that are actually in df_all
    # (excluding cbs_id, event, duration_days which are response/ID)
    structural_cols = [c for c in feat_structural.columns if c not in ["cbs_id"]]
    # Also add columns that are in survival_base but not in feat_structural
    survival_structural = ["Ekipman_Tipi", "Gerilim_Sinifi", "Marka"]
    for col in survival_structural:
        if col in df_all.columns and col not in structural_cols:
            structural_cols.append(col)

    temporal_cols = [c for c in feat_temporal.columns if c not in ["cbs_id"]]
    
    # Verify Kurulum_Tarihi present (should be from survival_base only)
    if "Kurulum_Tarihi" not in df_all.columns:
        logger.error("[DATASET] CRITICAL: Kurulum_Tarihi missing after merge!")

    logger.info(f"[DATASET] Final: {len(df_all)} rows × {len(df_all.columns)} cols")
    logger.info(f"[DATASET] Has Kurulum_Tarihi: {'Kurulum_Tarihi' in df_all.columns}")
    logger.info(f"[DATASET] Structural columns defined: {structural_cols}")
    logger.info(f"[DATASET] Temporal columns defined: {temporal_cols}")
    logger.info(f"[DATASET] Available columns in df_all: {list(df_all.columns)}")
    ...
    for eq_type in sorted(df_all["Ekipman_Tipi"].unique()):
        ...
        if stats["n_total"] < MIN_SAMPLES or stats["n_events"] < MIN_EVENTS:
            ...
            try:
                ...
                # Only use columns that exist in BOTH global model and current equipment
                shared_cols = [c for c in global_models["X_cox_cols"] if c in X_eq.columns]
                
                if len(shared_cols) < 3:
                    logger.warning(f"[{eq_type}] Insufficient shared features, skipping")
                    preds["Model_Type"] = "No_Prediction"
                    all_predictions.append(preds)
                    continue
                
                X_eq = X_eq[shared_cols]
    ...
    report = report.merge(predictions, on="cbs_id", how="left")
