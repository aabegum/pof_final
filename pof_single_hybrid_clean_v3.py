...
def add_temporal_features_inplace(
    ...
    # --- NEW: Observability Features for XGBoost/RSF ---
    # How long have we actually watched this asset?
    df["Observed_Duration_Days"] = (t_ref - np.maximum(df["Kurulum_Tarihi"], OBSERVATION_START_DATE)).dt.days
    ...
    # New assets = 1.0 (100%), Old assets = small % (high uncertainty)
    df["Observation_Ratio"] = (df["Observed_Duration_Days"] / df["Tref_Yas_Gun"]).fillna(1.0).clip(0, 1)
